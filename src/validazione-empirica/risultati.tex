...
Il dataset \textbf{quovadis} presenta tre classi \textit{Goodware}, \textit{Malware} e \textit{Backdoor}.\
Entrambi i modelli prestano ottime prestazioni nella classificazione dei \textit{Backdoor}, tuttavia\
si osserva una differenza significativa nel comportamento: \textit{XGBoost} tende a classificare più esempi\
di \textit{Backdoor} come \textit{Malware}.\
Sulla distinzione tra \textit{Malware} e \textit{Goodware}, \textit{XGBoost} è più conservativo classificando\
più \textit{Goodware} come \textit{Malware} ($1047$ contro i $251$) mentre \textit{RandomForest} mantiene un equilibrio migliore.
