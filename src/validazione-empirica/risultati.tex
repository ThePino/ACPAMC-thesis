...
\newcommand{\recall}[0]{\textit{Recall}}
\newcommand{\precision}[0]{\textit{Precision}}
\newcommand{\clscaption}[2]{\precision, \recall{} e \fiscore{} sul dataset {\textbf{#1}} con modello {\textit{#2}}}
\newcommand{\grpdescription}[1]{\textit{Overall Accuracy}, \textit{Micro} e \textit{Macro} metriche sul dataset \textbf{#1}}
\newcommand{\classmetrics}[3]{In \autoref{#1} e in \autoref{#2} le metriche \precision, \recall{} e \fiscore{} per \textit{XGBoost} e \textit{RandomForest} per il dataset \textbf{#3}.}
...
In generale, quando il modello predice erroneamente, tende a classificare i campioni come appartenenti alla classe \textit{Trojan}.\
...
In \autoref{fig:apimds-cls-xgb} vengono illustrate le metriche \precision, \recall{} e \fiscore{} per il modello \textit{XGBoost}.\
Per quanto riguarda la \precision, il modello presenta valori generalmente buoni, con la maggior parte delle classi che supera il punteggio di $0.73$.\
...
\
Relativamente alla \recall, si notano valori più variabili: il modello eccelle nella classificazione di \textit{Trojan}, ottenendo un punteggio di $0.94$.\
...
Questo modello mantiene valori di \precision{} elevati, superando la soglia di $0.80$ nella maggior parte delle classi (ad esempio, \textit{Malware} $0.89$ e \textit{Trojan} $0.87$).\
...
Per quanto concerne la \recall{}, si osserva un trend analogo al modello precedente, con prestazioni eccellenti su \textit{Trojan} ($0.93$) e \textit{Virus} ($0.85$).\
...
I vantaggi più evidenti si registrano nelle classi \textit{Virus} (\fiscore{} di $0.86$ contro $0.81$) e \textit{Packed} (\fiscore{} di $0.80$ contro $0.75$).\
...
Ad esempio in termini di \textit{Overall Accuracy} il modello \textit{RandomForest} ottiene un valore di $0.83$ contro il $0.81$ di \textit{XGBoost}.\
...
La classe \textit{Spyware} rappresenta il punto critico maggiore, registrando sia una \precision{} bassa ($0.53$) che una \recall{} insufficiente ($0.38$).\
...
\
Dal confronto diretto sul dataset \texttt{octack} emerge comunque un chiaro vantaggio del modello \textit{RandomForest}.
...
raggiungendo nelle metriche \precision, \recall{} e \fiscore{} valori superiori o uguali a $0.95$.\
Sulla classe \textit{Goodware} nonostante si abbia una \precision{} elevata ($0.95$), la \recall{} è pari a $0.79$ indicando che il modello\
fallisce nell'identificare oltre il \percc{20} dei campioni legitimi.\
Per la classe \textit{Malware}, il valore di \precision{} è il più basso  ($0.82$), suggerendo un tasso più elevato di falsi positivi rispetto alle altre due classi.
...
È interessante notare come, per entrambi i modelli, la classe \textit{Goodware} registri i valori più bassi di \recall{} e \fiscore{} rispetto alle altre classi.
